        ...
        <table className="w-full border-collapse text-left">
            <thead>
                <tr className={`border-b border-slate-500 text-slate-600 ${tableType === "panelAdmin" && "bg-slate-900 text-white"}`}>
                    ...
                    <th className="p-4 font-bold tetx-sm">Película</th>
                    ...
                    data?.map((item, index) => {
                        return (
                            <tr key={index} className="border-b border-salte-100 hover:bg-slate-50">
                                {
                                    tableType === "panelAdmin" &&
                                    <td className="p-4 font-bold text-red-600 text-sm text-left">{item?.userEmail}</td>
                                }
                                <td className="p-4 flex items-center gap-4">
                                    <div>
                                        <img src={item.movieImage} alt="pelicula" className="w-12 h-12 object-cover rounded-xs" />
                                    </div>
                                ...
                                <td className="p-4 font-black text-red-600 text-right">S/. {item?.price}</td>
                            </tr>
                        )
                    })
                }
